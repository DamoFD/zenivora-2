...
<section class="w-full min-h-screen relative overflow-hidden justify-center items-center flex flex-col" id="hero">
    @if (request()->routeIs('home'))
        {{--<img src="{{ asset('img/' . $img) }}" class="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 h-3/4" />--}}
    @else
        <picture class="absolute w-full object-cover h-screen bottom-0 left-0">
            <source srcset="{{ asset('img/' . $information->img) }}" media="(min-width: 600px)" />
            <img class="w-full h-screen object-cover" src="{{ asset('img/' . $information->img_mobile) }}" alt="{{ $information->title }}" />
        </picture>
    @endif
